/**
 * TierUpModal — web fallback
 *
 * Identical to TierUpModal.tsx except react-native-confetti-cannon is replaced
 * with a lightweight emoji-sparkle effect driven by useState + setTimeout.
 * expo-haptics is a no-op on web so we keep the import but it won't throw.
 * react-native-reanimated works on web, so all animations are preserved.
 *
 * Mobile (iOS/Android) loads TierUpModal.tsx — this file is web-only.
 */
...
// A single sparkle particle rendered at an absolute position
interface Sparkle {
  id: number;
  x: number;
  y: number;
...
export function TierUpModal({ visible, fromLevel, toLevel, onClose }: Props) {
  ...
  useEffect(() => {
    if (visible && toCfg && fromCfg && fromLevel !== toLevel) {
      ...
      // Show sparkles after a short delay (mirrors the confetti autoStart delay)
      sparkleTimerRef.current = setTimeout(() => {
        setSparkles(generateSparkles(18));
        // Auto-clear sparkles after animation window
        sparkleTimerRef.current = setTimeout(() => setSparkles([]), 2200);
      }, 100);
    }
    return () => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Emoji sparkles — rendered above the card, pointer-events-none equivalent */}
        {sparkles.map((s) => (
          <Text
            key={s.id}
