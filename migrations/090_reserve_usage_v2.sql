-- Migration 090: 3-tier consumption (included → pack → overage)
-- Rewrites public.reserve_usage and public.release_usage to consult the
-- credit_ledger (pack credits) between the included quota and overage tiers.
-- Updates public.aggregate_confirmed_usage to only bill the overage tier via
-- Stripe.
--
-- Depends on migrations 081, 082, 084, and whichever migration introduced
-- public.credit_ledger (pool + delta + reason + usage_event_id columns).
--
...
-- `SELECT ... INTO` inside plpgsql. Paste the whole file as ONE request.
--
-- NOTE: reserve_usage currently assumes p_quantity = 1 per call (the only
-- shape the edge functions send). Larger quantities are rejected downstream
-- by funding-source bookkeeping being per-unit; leave expansion for a later
-- migration.

BEGIN;

-- ============================================================
-- 1. reserve_usage  (3-tier: included → pack → overage)
-- ============================================================
...
BEGIN
  ...
  -- NOTE: 3-tier funding decision below is computed for the single unit the
  -- edge function always sends. If you ever raise p_quantity > 1 you must
  -- split the allocation across tiers explicitly.
  ...
  -- --------------------------------------------------------------
  -- Funding-source decision (per unit — p_quantity is forced to 1)
  -- --------------------------------------------------------------
  ...
  ELSE
    ...
    ELSE
      ...
      -- Cap check only matters for overage-funded units
      v_existing_over := (
        SELECT COALESCE(SUM(
  ...
  -- --------------------------------------------------------------
  -- Insert usage_events row with funding_source
  -- --------------------------------------------------------------
  ...
  -- Atomically decrement pack credit if funding is 'pack'
  IF v_funding_source = 'pack' THEN
    INSERT INTO public.credit_ledger (
      salon_id, pool, delta, reason, usage_event_id
    )
    VALUES (
      p_salon_id, v_pool, -p_quantity, 'consumption', v_reservation_id
    );
...
-- ============================================================
-- 2. release_usage  (refunds pack credits when applicable)
-- ============================================================
...
BEGIN
  ...
  -- Map sku -> credit pool (only needed if refunding pack credit)
  v_pool := CASE v_sku
    WHEN 'sms_reminder'    THEN 'sms_reminder'
    WHEN 'sms_marketing'   THEN 'sms_marketing'
    WHEN 'email_reminder'  THEN 'email'
  ...
  IF v_status = 'confirmed' THEN
    ...
    IF v_funding = 'pack' AND v_pool IS NOT NULL THEN
      ...
      VALUES (
        v_salon_id, v_pool, v_units, 'refund', p_reservation_id
      );
    END IF;
    RETURN;
  END IF;
...
-- ============================================================
-- 3. aggregate_confirmed_usage  (overage-only for Stripe billing)
-- ============================================================
