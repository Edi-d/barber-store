-- Migration 092: SMS Marketing bulk usage RPCs
-- Adds 4 SECURITY DEFINER RPCs for SMS marketing campaign flows:
--   1. estimate_usage_cost        -- pure read, STABLE
--   2. reserve_usage_bulk         -- atomic bulk reservation, partial-cap
--   3. release_usage_bulk         -- bulk release with pack-credit refunds
--   4. get_campaign_cost_summary  -- per-campaign aggregation (STABLE)
--
-- Depends on:
--   081 - metered_skus, usage_events, usage_periods, salon_billing_config
--   089 - credit_ledger (pool + delta + reason + usage_event_id), usage_events.funding_source
--   090 - reserve_usage v2 with 3-tier funding (included -> pack -> overage)
--   091 - salon_clients, sms_campaigns, sms_campaign_recipients
--
-- NOTE: Uses scalar subquery assignments (`v_X := (SELECT ... LIMIT 1)`)
-- throughout to sidestep the Supabase SQL Editor bug that mis-parses
-- `SELECT ... INTO` inside plpgsql. Uses the
-- `DROP FUNCTION IF EXISTS ...; CREATE FUNCTION ...` pattern.
...
BEGIN
  ...
  v_pool := CASE p_sku
    WHEN 'sms_reminder'    THEN 'sms_reminder'
    WHEN 'sms_marketing'   THEN 'sms_marketing'
    WHEN 'email_reminder'  THEN 'email'
  ...
  v_existing_over := COALESCE(v_existing_over, 0);

  v_cap_remaining    := GREATEST(0, v_cap_cents - v_existing_over);
  v_would_exceed_cap := (v_existing_over + v_overage_cents) > v_cap_cents AND v_overage_cents > 0;
  v_total_cost_cents := v_overage_cents;  -- what the user effectively pays
  ...
  RETURN jsonb_build_object(
    ...
    'would_exceed_cap',    v_would_exceed_cap,
...
BEGIN
  ...
  -- Idempotency short-circuit: if any row exists for this key, return its ids
  v_existing_count := (
    SELECT COUNT(*)::int
  ...
  -- Tiered allocation for the requested quantity
  v_included_units := LEAST(p_quantity, v_remaining_included);
  v_pack_units     := LEAST(p_quantity - v_included_units, v_pack_balance);
  v_overage_units  := p_quantity - v_included_units - v_pack_units;
  v_overage_cents  := v_overage_units * v_unit_price;

  -- Existing overage already reserved/confirmed this month (for cap math)
  v_existing_over := (
    SELECT COALESCE(SUM(
      GREATEST(0, (up.quantity_used_confirmed + up.quantity_reserved) - COALESCE(up.quota_included, 0))
      * ms.unit_price_cents
  ...
  -- Partial-reserve up to cap when hard_cap is enabled
  ...
  -- Insert all reservation rows in a single statement. Each gen row is
  -- mapped to its funding_source by its index in the allocation order:
  --   [1 .. included_units]                                -> 'included'
  --   [included_units+1 .. included_units+pack_units]      -> 'pack'
  --   [pack end+1 .. adjusted_qty]                          -> 'overage'
  WITH inserted AS (
  ...
  -- One summary credit_ledger row for the whole bulk op (pack tier only)
  IF v_pack_units > 0 AND v_pool IS NOT NULL THEN
    INSERT INTO public.credit_ledger (
      salon_id, pool, delta, reason, usage_event_id
    )
    VALUES (
      p_salon_id, v_pool, -v_pack_units, 'consumption', v_reservation_ids[1]
    );
  END IF;
...
-- ============================================================
-- 3. release_usage_bulk  (bulk refund w/ pack credit refunds)
-- ============================================================
...
BEGIN
  ...
  FOR v_rec IN
    SELECT id, salon_id, status::text AS status, units, period_id,
     ...
     FOR UPDATE
  LOOP
    IF v_rec.status IN ('released','expired','failed') THEN
      CONTINUE;
    END IF;
    ...
    IF v_rec.status = 'confirmed' THEN
      ...
      UPDATE public.usage_periods
         SET quantity_used_confirmed = GREATEST(0, quantity_used_confirmed - v_rec.units)
      ...
      IF v_rec.funding_source = 'pack' AND v_pool IS NOT NULL THEN
        ...
        VALUES (
          v_rec.salon_id, v_pool, v_rec.units, 'refund', v_rec.id
        );
      END IF;
...
-- ============================================================
-- 4. get_campaign_cost_summary  (STABLE aggregation)
-- ============================================================
...
DECLARE
  ...
  v_failed_count          int;
  v_total_cost_cents      int;
  v_from_included_units   int;
  v_from_pack_units       int;
  v_overage_units         int;
  v_overage_cents         int;
BEGIN
  v_total_reservations := (
    SELECT COUNT(*)::int
      FROM public.usage_events
     WHERE related_entity_type = 'campaign'
       AND related_entity_id   = p_campaign_id
  );
  ...
  v_from_included_units := (
    SELECT COALESCE(SUM(units), 0)::int
      FROM public.usage_events
     WHERE related_entity_type = 'campaign'
       AND related_entity_id   = p_campaign_id
       AND status              = 'confirmed'
       AND funding_source      = 'included'
  );
  ...
  RETURN jsonb_build_object(
    ...
    'total_cost_cents',      v_total_cost_cents,
    'from_included_units',   v_from_included_units,
    'from_pack_units',       v_from_pack_units,
    'overage_units',         v_overage_units,
    'overage_cents',         v_overage_cents
  );
END;
